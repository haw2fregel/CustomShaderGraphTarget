...
namespace UnityEditor.Rendering.Universal.ShaderGraph
{
    ...
    sealed class CustomUniversalTarget : Target, IHasMetadata, ILegacyTarget, IMaySupportVFX
#if HAS_VFX_GRAPH
        , IRequireVFXContext
#endif
    {
        ...
        // Constants
        static readonly GUID kSourceCodeGuid = new GUID("8c72f47fdde33b14a9340e325ce56f4d"); // CustomUniversalTarget.cs
        ...
        {
            // Core blocks
            if (!(m_ActiveSubTarget.value is UnityEditor.Rendering.Fullscreen.ShaderGraph.FullscreenSubTarget<CustomUniversalTarget>))
            {
        ...
        public override void GetPropertiesGUI(ref TargetPropertyGUIContext context, Action onChange, Action<String> registerUndo)
        {
            ...
            {
                // VFX Support
                if (!(m_ActiveSubTarget.value is CustomUniversalSubTarget))
        ...
        void ProcessSubTargetDatas(SubTarget subTarget)
        {
            ...
            foreach (var type in typeCollection)
            {
                ...
                var methodInfo = typeof(CustomUniversalTarget).GetMethod(nameof(SetDataOnSubTarget));
        ...
        void ClearUnusedData()
        {
            for (int i = 0; i < m_Datas.Count; i++)
            {
                ...
                // Data requirement interfaces need generic type arguments
                // Therefore we need to use reflections to call the method
                var methodInfo = typeof(CustomUniversalTarget).GetMethod(nameof(ValidateDataForSubTarget));
                var genericMethodInfo = methodInfo.MakeGenericMethod(type);
        ...
        public bool CanSupportVFX()
        {
            ...
            if (m_ActiveSubTarget.value is CustomUniversalLitSubTarget)
                return true;
        ...
        [Serializable]
        class CustomUniversalTargetLegacySerialization
        {
            [SerializeField]
        ...
        public override void OnAfterDeserialize(string json)
        {
            ...
            if (this.sgVersion < latestVersion)
            {
                if (this.sgVersion == 0)
                {
                    ...
                    var oldSettings = JsonUtility.FromJson<CustomUniversalTargetLegacySerialization>(json);
    ...
    static class CustomCorePasses
    {
        ...
        internal static void AddAlphaToMaskControlToPass(ref PassDescriptor pass, CustomUniversalTarget target)
        {
        ...
        internal static void AddAlphaClipControlToPass(ref PassDescriptor pass, CustomUniversalTarget target)
        {
            if (target.allowMaterialOverride)
                pass.keywords.Add(CustomCoreKeywordDescriptors.AlphaTestOn);
            else if (target.alphaClip)
                pass.defines.Add(CustomCoreKeywordDescriptors.AlphaTestOn, 1);
        }

        internal static void AddLODCrossFadeControlToPass(ref PassDescriptor pass, CustomUniversalTarget target)
        {
            if (target.supportsLodCrossFade)
            {
                pass.includes.Add(CustomCoreIncludes.LODCrossFade);
                pass.keywords.Add(CustomCoreKeywordDescriptors.LODFadeCrossFade);
                pass.defines.Add(CustomCoreKeywordDescriptors.UseUnityCrossFade, 1);
            }
        }

        internal static void AddTargetSurfaceControlsToPass(ref PassDescriptor pass, CustomUniversalTarget target, bool blendModePreserveSpecular = false)
        {
            // the surface settings can either be material controlled or target controlled
            if (target.allowMaterialOverride)
            {
                // setup material control of via keyword
                pass.keywords.Add(CustomCoreKeywordDescriptors.SurfaceTypeTransparent);
                pass.keywords.Add(CustomCoreKeywordDescriptors.AlphaPremultiplyOn);
                pass.keywords.Add(CustomCoreKeywordDescriptors.AlphaModulateOn);
            }
            else
            {
                ...
                {
                    pass.defines.Add(CustomCoreKeywordDescriptors.SurfaceTypeTransparent, 1);
                    ...
                    if ((target.alphaMode == AlphaMode.Alpha || target.alphaMode == AlphaMode.Additive) && blendModePreserveSpecular)
                        pass.defines.Add(CustomCoreKeywordDescriptors.AlphaPremultiplyOn, 1);
                    else if (target.alphaMode == AlphaMode.Multiply)
                        pass.defines.Add(CustomCoreKeywordDescriptors.AlphaModulateOn, 1);
                }
            }
        ...
        public static PassDescriptor DepthOnly(CustomUniversalTarget target)
        {
            var result = new PassDescriptor()
            {
                ...
                validPixelBlocks = CustomCoreBlockMasks.FragmentAlphaOnly,
                ...
                // Conditional State
                renderStates = CustomCoreRenderStates.DepthOnly(target),
                pragmas = CustomCorePragmas.Instanced,
                defines = new DefineCollection(),
                keywords = new KeywordCollection(),
                includes = new IncludeCollection { CustomCoreIncludes.DepthOnly },
        ...
        public static PassDescriptor DepthNormalOnly(CustomUniversalTarget target)
        {
            var result = new PassDescriptor()
            {
                ...
                pragmas = CustomCorePragmas.Instanced,
                defines = new DefineCollection(),
                keywords = new KeywordCollection { CustomCoreKeywordDescriptors.GBufferNormalsOct },
                includes = new IncludeCollection { CustomCoreIncludes.DepthNormalsOnly },
        ...
        public static PassDescriptor ShadowCaster(CustomUniversalTarget target)
        {
            var result = new PassDescriptor()
            {
                // Definition
                ...
                // Fields
                structs = CustomCoreStructCollections.Default,
                requiredFields = CustomCoreRequiredFields.ShadowCaster,
                fieldDependencies = CustomCoreFieldDependencies.Default,

                // Conditional State
                renderStates = CustomCoreRenderStates.ShadowCaster(target),
                pragmas = CustomCorePragmas.Instanced,
                defines = new DefineCollection(),
                keywords = new KeywordCollection { CustomCoreKeywords.ShadowCaster },
                includes = new IncludeCollection { CustomCoreIncludes.ShadowCaster },
        ...
        public static PassDescriptor SceneSelection(CustomUniversalTarget target)
        {
            var result = new PassDescriptor()
            {
                // Definition
                ...
                // Conditional State
                renderStates = CustomCoreRenderStates.SceneSelection(target),
                pragmas = CustomCorePragmas.Instanced,
                defines = new DefineCollection { CustomCoreDefines.SceneSelection, { CustomCoreKeywordDescriptors.AlphaClipThreshold, 1 } },
                keywords = new KeywordCollection(),
                includes = CustomCoreIncludes.SceneSelection,
        ...
        public static PassDescriptor ScenePicking(CustomUniversalTarget target)
        {
            var result = new PassDescriptor()
            {
                // Definition
                ...
                // Conditional State
                renderStates = CustomCoreRenderStates.ScenePicking(target),
                pragmas = CustomCorePragmas.Instanced,
                defines = new DefineCollection { CustomCoreDefines.ScenePicking, { CustomCoreKeywordDescriptors.AlphaClipThreshold, 1 } },
                keywords = new KeywordCollection(),
                includes = CustomCoreIncludes.ScenePicking,
        ...
        public static PassDescriptor _2DSceneSelection(CustomUniversalTarget target)
        {
            var result = new PassDescriptor()
            {
                // Definition
                ...
                // Conditional State
                renderStates = CustomCoreRenderStates.SceneSelection(target),
                pragmas = CustomCorePragmas._2DDefault,
                defines = new DefineCollection { CustomCoreDefines.SceneSelection, { CustomCoreKeywordDescriptors.AlphaClipThreshold, 0 } },
                keywords = new KeywordCollection(),
                includes = CustomCoreIncludes.ScenePicking,
        ...
        public static PassDescriptor _2DScenePicking(CustomUniversalTarget target)
        {
            var result = new PassDescriptor()
            {
                // Definition
                ...
                renderStates = CustomCoreRenderStates.ScenePicking(target),
                pragmas = CustomCorePragmas._2DDefault,
                defines = new DefineCollection { CustomCoreDefines.ScenePicking, { CustomCoreKeywordDescriptors.AlphaClipThreshold, 0 } },
                keywords = new KeywordCollection(),
                includes = CustomCoreIncludes.SceneSelection,
    ...
    static class CustomCoreStructCollections
    {
    ...
    static class CustomCoreRequiredFields
    {
    ...
    static class CustomCoreFieldDependencies
    {
    ...
    #region RenderStates
    static class CustomCoreRenderStates
    {
        ...
        // used by lit/unlit subtargets
        public static RenderStateCollection UberSwitchedRenderState(CustomUniversalTarget target, bool blendModePreserveSpecular = false)
        {
            if (target.allowMaterialOverride)
            {
        ...
        public static RenderStateDescriptor UberSwitchedCullRenderState(CustomUniversalTarget target)
        {
            if (target.allowMaterialOverride)
        ...
        // used by lit/unlit targets
        public static RenderStateCollection ShadowCaster(CustomUniversalTarget target)
        {
            var result = new RenderStateCollection
            {
                { RenderState.ZTest(ZTest.LEqual) },
        ...
        // used by lit/unlit targets
        public static RenderStateCollection DepthOnly(CustomUniversalTarget target)
        {
            var result = new RenderStateCollection
            {
                { RenderState.ZTest(ZTest.LEqual) },
        ...
        // used by lit target ONLY
        public static RenderStateCollection DepthNormalsOnly(CustomUniversalTarget target)
        {
            var result = new RenderStateCollection
            {
                { RenderState.ZTest(ZTest.LEqual) },
        ...
        // Used by all targets
        public static RenderStateCollection SceneSelection(CustomUniversalTarget target)
        {
            var result = new RenderStateCollection
            {
                { RenderState.Cull(Cull.Off) },
        ...
        public static RenderStateCollection ScenePicking(CustomUniversalTarget target)
        {
            var result = new RenderStateCollection
            {
                { UberSwitchedCullRenderState(target) }
    ...
    #region Pragmas
    static class CustomCorePragmas
    {
    ...
    #region Includes
    static class CustomCoreIncludes
    {
        ...
        const string kLighting = "Assets/CustomURP/ShaderLibraly/CustomLighting.hlsl";
    ...
    #region Defines
    static class CustomCoreDefines
    {
        public static readonly DefineCollection UseLegacySpriteBlocks = new DefineCollection
        {
            { CustomCoreKeywordDescriptors.UseLegacySpriteBlocks, 1, new FieldCondition(CustomCoreFields.UseLegacySpriteBlocks, true) },
        };

        public static readonly DefineCollection UseFragmentFog = new DefineCollection()
        {
            {CustomCoreKeywordDescriptors.UseFragmentFog, 1},
        };

        public static readonly DefineCollection SceneSelection = new DefineCollection
        {
            { CustomCoreKeywordDescriptors.SceneSelectionPass, 1 },
        };

        public static readonly DefineCollection ScenePicking = new DefineCollection
        {
            { CustomCoreKeywordDescriptors.ScenePickingPass, 1 },
        };
    }
    #endregion

    #region KeywordDescriptors
    static class CustomCoreKeywordDescriptors
    {
    ...
    #region Keywords
    static class CustomCoreKeywords
    {
        public static readonly KeywordCollection ShadowCaster = new KeywordCollection
        {
            { CustomCoreKeywordDescriptors.CastingPunctualLightShadow },
        };
    }
    #endregion

    #region FieldDescriptors
    static class CustomCoreFields
    {
    ...
    static class CustomCoreCustomInterpDescriptors
    {
