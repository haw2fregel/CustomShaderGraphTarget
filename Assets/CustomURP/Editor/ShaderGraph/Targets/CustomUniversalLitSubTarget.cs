...
{
    sealed class CustomUniversalLitSubTarget : CustomUniversalSubTarget, ILegacyTarget
    {
        static readonly GUID kSourceCodeGuid = new GUID("d6c78107b64145745805d963de80cc17"); // CustomUniversalLitSubTarget.cs
        ...
        public CustomUniversalLitSubTarget()
        {
            displayName = "Lit";
        }
        ...
        public override void Setup(ref TargetSetupContext context)
        {
            ...
            // Process CustomSubShaders
            context.AddSubShader(PostProcessSubShader(CustomSubShaders.LitSubShader(target, workflowMode, target.renderType, target.renderQueue, target.disableBatching, complexLit, blendModePreserveSpecular)));
        }
        ...
        {
            var CustomUniversalTarget = (target as CustomUniversalTarget);
            CustomUniversalTarget.AddDefaultMaterialOverrideGUI(ref context, onChange, registerUndo);
            ...
            CustomUniversalTarget.AddDefaultSurfacePropertiesGUI(ref context, onChange, registerUndo, showReceiveShadows: true);
        ...
        #region SubShader
        static class CustomSubShaders
        {
            public static SubShaderDescriptor LitSubShader(CustomUniversalTarget target, WorkflowMode workflowMode, string renderType, string renderQueue, string disableBatchingTag, bool complexLit, bool blendModePreserveSpecular)
            {
                SubShaderDescriptor result = new SubShaderDescriptor()
                {
                    pipelineTag = CustomUniversalTarget.kPipelineTag,
                    customTags = complexLit ? CustomUniversalTarget.kComplexLitMaterialTypeTag : CustomUniversalTarget.kLitMaterialTypeTag,
                ...
                if (complexLit)
                    result.passes.Add(CustomLitPasses.ForwardOnly(target, workflowMode, complexLit, blendModePreserveSpecular, CustomCoreBlockMasks.Vertex, CustomLitBlockMasks.FragmentComplexLit, CustomCorePragmas.Forward, CustomLitKeywords.Forward));
                else
                    result.passes.Add(CustomLitPasses.Forward(target, workflowMode, blendModePreserveSpecular, CustomCorePragmas.Forward, CustomLitKeywords.Forward));

                // ForwardOnly ComplexLit fills GBuffer too for potential custom usage of the GBuffer.
                result.passes.Add(CustomLitPasses.GBuffer(target, workflowMode, blendModePreserveSpecular));

                // cull the shadowcaster pass if we know it will never be used
                if (target.castShadows || target.allowMaterialOverride)
                    result.passes.Add(PassVariant(CustomCorePasses.ShadowCaster(target), CustomCorePragmas.Instanced));

                if (target.mayWriteDepth)
                    result.passes.Add(PassVariant(CustomCorePasses.DepthOnly(target), CustomCorePragmas.Instanced));

                if (complexLit)
                    result.passes.Add(PassVariant(CustomLitPasses.DepthNormalOnly(target), CustomCorePragmas.Instanced));
                else
                    result.passes.Add(PassVariant(CustomLitPasses.DepthNormal(target), CustomCorePragmas.Instanced));

                result.passes.Add(PassVariant(CustomLitPasses.Meta(target), CustomCorePragmas.Default));

                // Currently neither of these passes (selection/picking) can be last for the game view for
                // UI shaders to render correctly. Verify [1352225] before changing this order.
                result.passes.Add(PassVariant(CustomCorePasses.SceneSelection(target), CustomCorePragmas.Default));
                result.passes.Add(PassVariant(CustomCorePasses.ScenePicking(target), CustomCorePragmas.Default));
                result.passes.Add(PassVariant(CustomLitPasses._2D(target), CustomCorePragmas.Default));
        ...
        {
            static void AddWorkflowModeControlToPass(ref PassDescriptor pass, CustomUniversalTarget target, WorkflowMode workflowMode)
            {
                if (target.allowMaterialOverride)
                    pass.keywords.Add(CustomLitDefines.SpecularSetup);
                else if (workflowMode == WorkflowMode.Specular)
                    pass.defines.Add(CustomLitDefines.SpecularSetup, 1);
            }

            static void AddReceiveShadowsControlToPass(ref PassDescriptor pass, CustomUniversalTarget target, bool receiveShadows)
            {
                if (target.allowMaterialOverride)
                ...
                KeywordCollection keywords)
            {
                var result = new PassDescriptor()
                {
                    ...
                    // Template
                    passTemplatePath = CustomUniversalTarget.kUberTemplatePath,
                    sharedTemplateDirectories = CustomUniversalTarget.kSharedTemplateDirectories,

                    // Port Mask
                    validVertexBlocks = CustomCoreBlockMasks.Vertex,
                    validPixelBlocks = CustomLitBlockMasks.FragmentLit,

                    // Fields
                    structs = CustomCoreStructCollections.Default,
                    requiredFields = CustomLitRequiredFields.Forward,
                    fieldDependencies = CustomCoreFieldDependencies.Default,

                    // Conditional State
                    renderStates = CustomCoreRenderStates.UberSwitchedRenderState(target, blendModePreserveSpecular),
                    pragmas = pragmas ?? CustomCorePragmas.Forward,     // NOTE: SM 2.0 only GL
                    defines = new DefineCollection() { CustomCoreDefines.UseFragmentFog },
                    keywords = new KeywordCollection() { keywords },
                    includes = CustomLitIncludes.Forward,

                    // Custom Interpolator Support
                    customInterpolators = CustomCoreCustomInterpDescriptors.Common
                };

                CustomCorePasses.AddTargetSurfaceControlsToPass(ref result, target, blendModePreserveSpecular);
                CustomCorePasses.AddAlphaToMaskControlToPass(ref result, target);
                ...
                CustomCorePasses.AddLODCrossFadeControlToPass(ref result, target);
                ...
                KeywordCollection keywords)
            {
                var result = new PassDescriptor
                {
                    ...
                    defines = new DefineCollection { CustomCoreDefines.UseFragmentFog },
                    keywords = new KeywordCollection { keywords },
                    includes = new IncludeCollection { CustomLitIncludes.Forward },
                ...
                if (complexLit)
                    result.defines.Add(CustomLitDefines.ClearCoat, 1);
            ...
            public static PassDescriptor GBuffer(CustomUniversalTarget target, WorkflowMode workflowMode, bool blendModePreserveSpecular)
            {
                var result = new PassDescriptor
                {
                    // Definition
                    ...
                    // Fields
                    structs = CustomCoreStructCollections.Default,
                    requiredFields = CustomLitRequiredFields.GBuffer,
                    fieldDependencies = CustomCoreFieldDependencies.Default,

                    // Conditional State
                    renderStates = CustomCoreRenderStates.UberSwitchedRenderState(target, blendModePreserveSpecular),
                    pragmas = CustomCorePragmas.GBuffer,
                    defines = new DefineCollection { CustomCoreDefines.UseFragmentFog },
                    keywords = new KeywordCollection { CustomLitKeywords.GBuffer },
                    includes = new IncludeCollection { CustomLitIncludes.GBuffer },
            ...
            public static PassDescriptor Meta(CustomUniversalTarget target)
            {
                var result = new PassDescriptor()
                {
                    // Definition
                    ...
                    // Port Mask
                    validVertexBlocks = CustomCoreBlockMasks.Vertex,
                    validPixelBlocks = CustomLitBlockMasks.FragmentMeta,

                    // Fields
                    structs = CustomCoreStructCollections.Default,
                    requiredFields = CustomLitRequiredFields.Meta,
                    fieldDependencies = CustomCoreFieldDependencies.Default,

                    // Conditional State
                    renderStates = CustomCoreRenderStates.Meta,
                    pragmas = CustomCorePragmas.Default,
                    defines = new DefineCollection() { CustomCoreDefines.UseFragmentFog },
                    keywords = new KeywordCollection() { CustomCoreKeywordDescriptors.EditorVisualization },
                    includes = CustomLitIncludes.Meta,

                    // Custom Interpolator Support
                    customInterpolators = CustomCoreCustomInterpDescriptors.Common
                };

                CustomCorePasses.AddAlphaClipControlToPass(ref result, target);

                return result;
            }

            public static PassDescriptor _2D(CustomUniversalTarget target)
            {
                var result = new PassDescriptor()
                {
                    // Definition
                    ...
                    // Port Mask
                    validVertexBlocks = CustomCoreBlockMasks.Vertex,
                    validPixelBlocks = CustomCoreBlockMasks.FragmentColorAlpha,
                    ...
                    // Conditional State
                    renderStates = CustomCoreRenderStates.UberSwitchedRenderState(target),
                    pragmas = CustomCorePragmas.Instanced,
                    defines = new DefineCollection(),
                    keywords = new KeywordCollection(),
                    includes = CustomLitIncludes._2D,
            ...
            public static PassDescriptor DepthNormal(CustomUniversalTarget target)
            {
                var result = new PassDescriptor()
                {
                    // Definition
                    ...
                    // Port Mask
                    validVertexBlocks = CustomCoreBlockMasks.Vertex,
                    validPixelBlocks = CustomCoreBlockMasks.FragmentDepthNormals,

                    // Fields
                    structs = CustomCoreStructCollections.Default,
                    requiredFields = CustomCoreRequiredFields.DepthNormals,
                    fieldDependencies = CustomCoreFieldDependencies.Default,

                    // Conditional State
                    renderStates = CustomCoreRenderStates.DepthNormalsOnly(target),
                    pragmas = CustomCorePragmas.Instanced,
                    defines = new DefineCollection(),
                    keywords = new KeywordCollection(),
                    includes = new IncludeCollection { CustomCoreIncludes.DepthNormalsOnly },
            ...
            public static PassDescriptor DepthNormalOnly(CustomUniversalTarget target)
            {
                var result = new PassDescriptor()
                {
                    // Definition
        ...
        static class CustomLitKeywords
        {
            ...
            {
                { CustomCoreKeywordDescriptors.ScreenSpaceAmbientOcclusion },
                { CustomCoreKeywordDescriptors.StaticLightmap },
                { CustomCoreKeywordDescriptors.DynamicLightmap },
                { CustomCoreKeywordDescriptors.DirectionalLightmapCombined },
                { CustomCoreKeywordDescriptors.MainLightShadows },
                { CustomCoreKeywordDescriptors.AdditionalLights },
                { CustomCoreKeywordDescriptors.AdditionalLightShadows },
                { CustomCoreKeywordDescriptors.ReflectionProbeBlending },
                { CustomCoreKeywordDescriptors.ReflectionProbeBoxProjection },
                { CustomCoreKeywordDescriptors.ShadowsSoft },
                { CustomCoreKeywordDescriptors.LightmapShadowMixing },
                { CustomCoreKeywordDescriptors.ShadowsShadowmask },
                { CustomCoreKeywordDescriptors.DBuffer },
                { CustomCoreKeywordDescriptors.LightLayers },
                { CustomCoreKeywordDescriptors.DebugDisplay },
                { CustomCoreKeywordDescriptors.LightCookies },
                { CustomCoreKeywordDescriptors.ForwardPlus },
                { CustomCoreKeywordDescriptors.EvaluateSh },
            };
            ...
            public static readonly KeywordCollection GBuffer = new KeywordCollection
            {
                ...
                { CustomCoreKeywordDescriptors.LightmapShadowMixing },
                { CustomCoreKeywordDescriptors.ShadowsShadowmask },
                { CustomCoreKeywordDescriptors.MixedLightingSubtractive },
                { CustomCoreKeywordDescriptors.DBuffer },
                { CustomCoreKeywordDescriptors.GBufferNormalsOct },
                { CustomCoreKeywordDescriptors.RenderPassEnabled },
                { CustomCoreKeywordDescriptors.DebugDisplay },
            };
        }
        #endregion
        ...
        static class CustomLitIncludes
        {
            ...
            const string kForwardPass = "Assets/CustomURP/Editor/ShaderGraph/includes/CustomPBRForwardPass.hlsl";
            ...
            {
                // Pre-graph
                { CustomCoreIncludes.DOTSPregraph },
                { CustomCoreIncludes.WriteRenderLayersPregraph },
                { CustomCoreIncludes.CorePregraph },
                { kShadows, IncludeLocation.Pregraph },
                { CustomCoreIncludes.ShaderGraphPregraph },
                { CustomCoreIncludes.DBufferPregraph },

                // Post-graph
                { CustomCoreIncludes.CorePostgraph },
